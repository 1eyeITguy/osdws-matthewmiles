function Remove-OSDWorkspaceWinPEBuild {
    ...
    begin {
        ...
        # Import OSD.Workspace module if not already loaded
        if (-not (Get-Module -Name OSD.Workspace)) {
            try {
                Import-Module OSD.Workspace -ErrorAction Stop
            }
            catch {
                Write-Error "[$(Get-Date -format G)] [$($MyInvocation.MyCommand.Name)] Failed to import OSD.Workspace module: $($_.Exception.Message)"
                return
            }
        }
        ...
        #=================================================
        # Get OSD.Workspace settings
        if (-not $global:OSDWorkspace) {
            $global:OSDWorkspace = Get-OSDWorkspace
        }
        
        # Validate OSDWorkspace path exists
        if (-not $global:OSDWorkspace -or -not $global:OSDWorkspace.path) {
            Write-Error "[$(Get-Date -format G)] [$($MyInvocation.MyCommand.Name)] OSDWorkspace is not properly configured. Please run Install-OSDWorkspace first."
            return
        }
        #=================================================
    }
    ...
    process {
        ...
        # Get the WinPE build path
        $WinPEBuildPath = Join-Path $OSDWorkspace.path "build\windows-pe"
        
        if (-not (Test-Path $WinPEBuildPath)) {
            Write-Warning "[$(Get-Date -format G)] [$($MyInvocation.MyCommand.Name)] WinPE build directory not found: $WinPEBuildPath"
            return
        }

        # Get all build directories
        $BuildDirectories = Get-ChildItem -Path $WinPEBuildPath -Directory

        if (-not $BuildDirectories) {
            Write-Warning "[$(Get-Date -format G)] [$($MyInvocation.MyCommand.Name)] No WinPE builds found in $WinPEBuildPath"
            return
        }

        # Create build objects with properties
        $AvailableBuilds = foreach ($BuildDir in $BuildDirectories) {
            $PropertiesFile = Join-Path $BuildDir.FullName "properties.json"
            
            if (Test-Path $PropertiesFile) {
                $Properties = Get-Content $PropertiesFile -Raw | ConvertFrom-Json
                
                # Filter by architecture if specified
                if ($Architecture -and $Properties.Architecture -ne $Architecture) {
                    continue
                }
                ...
                [PSCustomObject]@{
                    ...
                    ModifiedTime = $BuildDir.LastWriteTime
                    Path = $BuildDir.FullName
                }
            }
            else {
                # If no properties file, create a basic object
                if ($Architecture) {
                    # Try to determine architecture from directory name
                    if ($BuildDir.Name -notmatch $Architecture) {
                        continue
                    }
                }
                ...
                [PSCustomObject]@{
                    Name = $BuildDir.Name
                    Id = $BuildDir.Name
                    Architecture = if ($BuildDir.Name -match 'amd64') { 'amd64' } elseif ($BuildDir.Name -match 'arm64') { 'arm64' } else { 'Unknown' }
                    Version = 'Unknown'
        ...
        if ($Force -eq $true) {
            ...
            try {
                $IndexPath = Join-Path $WinPEBuildPath "index.json"
                if (Test-Path $IndexPath) {
                    # Regenerate the index based on remaining builds
                    $RemainingBuilds = Get-ChildItem -Path $WinPEBuildPath -Directory
                    $IndexData = @{
                        builds = @($RemainingBuilds.Name)
                        lastUpdated = (Get-Date -Format "yyyy-MM-ddTHH:mm:ss")
                    }
                    $IndexData | ConvertTo-Json | Set-Content -Path $IndexPath -Force
                    Write-Host -ForegroundColor Green "[$(Get-Date -format G)] [$($MyInvocation.MyCommand.Name)] WinPE build index updated successfully"
                }
                else {
                    Write-Verbose "[$(Get-Date -format G)] [$($MyInvocation.MyCommand.Name)] No index file found to update"
                }
            }
            catch {
